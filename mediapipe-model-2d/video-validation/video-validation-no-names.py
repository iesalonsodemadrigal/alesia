...
json_path = os.path.join("output", "hola3.9.3.json")
...
while True:
    ...
    if frame_index < total_frames:
        ...
        for key, indices in FACE_CONNECTIONS.items():
            for i in range(len(indices) - 1):
                point1, point2 = f"mixamorig2:{key}_{indices[i]}", f"mixamorig2:{key}_{indices[i+1]}"
                if point1 in bone_positions and point2 in bone_positions:
                    cv2.line(frame, bone_positions[point1], bone_positions[point2], (255, 105, 180), 2)
